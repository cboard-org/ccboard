...
D) On Xcode, open `AAC Cboard.xcworkspace` that is located under `ccboard/platforms/ios` (The `.xcworkspace` file is generated by CocoaPods during `pod install` — make sure to use it instead of the `.xcodeproj` file)
