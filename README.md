...
D) On Xcode, open `AAC Cboard.xcworkspace` that is located under `ccboard/platforms/ios` (The `.xcworkspace` file is created automatically by Cordova/CocoaPods during `cordova platform add ios` — make sure to use it instead of the `.xcodeproj` file)
...
6. Start the active scheme with the 'play' button in the top left corner
